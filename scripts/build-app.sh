...
rm -f "$ROOT_DIR/dist/$APP_NAME-$VERSION-macos-arm64.dmg"
DMG_STAGE="$(mktemp -d)"
cp -R "$APP_DIR" "$DMG_STAGE/$APP_NAME.app"
ln -s /Applications "$DMG_STAGE/Applications"
hdiutil create -volname "$APP_NAME $VERSION" -srcfolder "$DMG_STAGE" -ov -format UDZO "$ROOT_DIR/dist/$APP_NAME-$VERSION-macos-arm64.dmg" >/dev/null
rm -rf "$DMG_STAGE"
print "Built $APP_DIR"
print "Built $ROOT_DIR/dist/$APP_NAME-$VERSION-macos-arm64.zip"
print "Built $ROOT_DIR/dist/$APP_NAME-$VERSION-macos-arm64.dmg"
